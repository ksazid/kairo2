...
function requireComparableControl(control: HunterShadowControlLaneResult): void {
  if (!isHunterShadowControlComparable(control)) {
    throw new Error(
      "Comparable Hunter V1 control requires non-zero evidence, measured model execution, no hunter-model degradation and positive measured cost",
    );
  }
}
